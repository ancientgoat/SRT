 ...
 */
public abstract class JdbcTemplateRepository<T extends Persistable<ID>, ID extends Serializable>
	...
	public JdbcTemplateRepository(RowMapper<T> rowMapper, RowUnmapper<T> rowUnmapper,
	...
	public JdbcTemplateRepository(RowMapper<T> rowMapper, RowUnmapper<T> rowUnmapper, String tableName,
			String idColumn) {
	...
	public JdbcTemplateRepository(RowMapper<T> rowMapper, TableDescription table) {
		this(rowMapper, new MissingRowUnmapper<T>(), null, table);
	}

	public JdbcTemplateRepository(RowMapper<T> rowMapper, String tableName, String idColumn) {
		this(rowMapper, new MissingRowUnmapper<T>(), null,
				new TableDescription(tableName, idColumn));
	}

	public JdbcTemplateRepository(RowMapper<T> rowMapper, String tableName) {
	...
	public boolean exists(ID id) {
		return jdbcOperations.queryForObject(sqlGenerator.countById(table), Integer.class,
				idToObjectArray(id)) > 0;
	}
	...
	public T findOne(ID id) {
		...
		final List<T> entityOrEmpty = jdbcOperations.query(sqlGenerator.selectById(table),
				idColumns, rowMapper);
	...
	public Iterable<T> findAll(Iterable<ID> ids) {
		...
		return jdbcOperations.query(sqlGenerator.selectByIds(table, idsList.size()), rowMapper,
				idColumnValues);
	}
